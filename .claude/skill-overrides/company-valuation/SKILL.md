...
# Look up the sector band for the company's sector/industry.
# wacc_lo, wacc_hi = sector_band_low, sector_band_high

# Step 4d-iii. Peer-implied WACC crosscheck
# For each peer, compute: implied_ke_peer = rf + beta_peer * erp
# then peer_wacc = e_v_peer*ke_peer + d_v_peer*kd_peer*(1-tax_peer)
# peer_wacc_median = median of peer WACCs
# Also back-solve from market price: market_implied_ke ≈ (1/fwd_pe) + long_run_growth
# where long_run_growth = terminal g (2.5-4%) — NOT the current-year consensus growth

# Step 4d-iv. Adjudication logic — produce a SINGLE wacc with a written rationale:
#
#   IF wacc_formula is INSIDE the sector sanity band:
#       wacc = wacc_formula   # formula wins, no adjustment needed
#       note = "Formula WACC within sector range"
#
#   IF wacc_formula is OUTSIDE the sector sanity band:
#       # Diagnose WHY it is out of range. Common causes:
#       # (A) Company is near-100% equity financed → debt tax shield negligible,
#       #     formula collapses to pure ke = rf + beta*erp, which is high.
#       #     Remedy: check if peer-implied WACC and market-implied ke are also high.
#       #     If peers also compute to similar ke, formula is correct; flag the sector
#       #     band as too wide / stale and use formula.
#       # (B) Beta is elevated due to short history, leverage history, or illiquidity.
#       #     Remedy: use sector-default beta from references/wacc_erp_rates.md
#       #     and recompute ke_adj = rf + beta_sector * erp.
#       # (C) Company's franchise quality / FCF durability argues for a lower risk premium
#       #     (e.g., mega-cap, investment-grade, >40% FCF margin, >80% recurring revenue).
#       #     Remedy: apply a "quality haircut" of 50-150bps to ke before blending.
#       #     Document the haircut and its basis explicitly.
#       #
#       # After diagnosis, choose ONE of:
#       #   wacc = wacc_formula          # if diagnosis says formula is right despite range
#       #   wacc = wacc_sector_midpoint  # if beta or capital structure is the distortion
#       #   wacc = 0.5*wacc_formula + 0.5*peer_wacc_median  # blend when uncertain
#       #
#       # ALWAYS run the sensitivity grid with WACC starting from formula value
#       # AND show the sector-midpoint column so the reader can see both anchors.
#       note = "Formula WACC outside sector range — see adjudication note in output"

# Final assignment (replace placeholder with actual adjudication result above):
wacc = wacc_formula   # updated by adjudication logic above
# REQUIRED in output: a "WACC Adjudication" box showing:
#   formula_wacc | sector_band | peer_median_wacc | market_implied_ke | final_wacc | reason
...
**Gates:** (a) if `wacc <= g_terminal` → stop, g too aggressive; (b) if `pv_tv / ev > 0.85` or `< 0.45` → flag and show both TV methods; (c) if `wacc_formula` is outside the sector sanity band → run Step 4d adjudication, do not silently use the out-of-range value.
